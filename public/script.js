...
const ErrorHandler = {
    ...
    show(error, fallbackMessage = 'Terjadi kesalahan') {
        ...
        if (error.recoverable) {
            ...
            this.showErrorModal(message, error.stack);
        }
    },
    ...
    showErrorModal(message, details) {
        ...
        if (!modal) {
            ...
            modal.innerHTML = `
                <div class="modal-box error-modal">
                    <i class="fa-solid fa-circle-exclamation" style="font-size: 48px; color: #e74c3c; margin-bottom: 20px;"></i>
                    <h3>Oops! Ada Masalah</h3>
                    <p id="error-message" style="margin-bottom: 20px; color: #ddd;"></p>
                    <details style="margin-bottom: 20px; text-align: left;">
                        <summary style="color: #888; cursor: pointer;">Detail Teknis</summary>
                        <pre id="error-details" style="background: #222; padding: 10px; border-radius: 5px; margin-top: 10px; font-size: 11px; overflow: auto; color: #f88;"></pre>
                    </details>
                    <div class="modal-btns">
                        <button onclick="location.reload()" class="btn-save" style="background: #e74c3c;">Muat Ulang</button>
                        <button onclick="this.closest('.modal-overlay').classList.remove('active')" class="btn-cancel">Tutup</button>
                    </div>
                </div>
            `;
        ...
        modal.querySelector('#error-details').textContent = details || 'Tidak ada detail';
    },
...
class CacheSystem {
    ...
    async set(store, key, value, ttl = 7 * 24 * 60 * 60 * 1000) {
        if (!this.db) await this.initDB();
        
        const size = new Blob([JSON.stringify(value)]).size;
        if (size > this.maxSize / 10) {
            console.warn('Item terlalu besar untuk cache:', key);
            return;
        }
    ...
    async cleanup(store) {
        ...
        const now = Date.now();
        const expiredRequest = index.openCursor(IDBKeyRange.upperBound(now - 7 * 24 * 60 * 60 * 1000));
        ...
        sizeRequest.onsuccess = (event) => {
            ...
            if (cursor) {
                ...
                this.removeOldest(store);
            }
        };
    }
    
    async removeOldest(store) {
        const transaction = this.db.transaction([store], 'readwrite');
        const objectStore = transaction.objectStore(store);
        const index = objectStore.index('timestamp');
...
const imageCache = new CacheSystem('images');
...
function handleAudioError(e) {
    ...
    if (audioLoadAttempts < MAX_RETRY_ATTEMPTS) {
        ...
        setTimeout(() => {
            if (currentMeta) {
                audio.src = `/api/index?url=${encodeURIComponent(currentMeta.url)}&mode=stream`;
                audio.load();
...
async function suggestNextSong() {
    ...
    try {
        const response = await fetchWithRetry(`/api/index?url=${encodeURIComponent(currentMeta.artist)}&mode=search`);
...
async function fetchWithRetry(url, options = {}, retries = 3) {
    ...
    for (let i = 0; i < retries; i++) {
        try {
            ...
            if (!response.ok) {
                throw new AppError(`HTTP ${response.status}: ${response.statusText}`, 'network', true);
            }
            
            return response;
        } catch (error) {
            if (error.name === 'AbortError') {
                throw new AppError('Request timeout', 'network', true);
            }
...
async function performSearch(query) {
    ...
    try {
        ...
        const response = await fetchWithRetry(`/api/index?url=${encodeURIComponent(query)}&mode=search`, {
            timeout: 15000
        });
...
function renderSearchResults(songs) {
    ...
    songs.forEach(song => {
        ...
        item.innerHTML = `
            <img src="${song.thumbnail}" alt="art" loading="lazy">
...
async function downloadCurrentSong() {
    ...
    try {
        ...
        const response = await fetchWithRetry(`/api/index?url=${encodeURIComponent(currentMeta.url)}&mode=stream`, {
            timeout: 60000
        });
...
async function syncUserData() {
    ...
    try {
        ...
        showToast('Gagal sinkronisasi', 'warning');
    }
}
...
function loadLibrary() {
    ...
    playlists.forEach(pl => {
        ...
        item.className = 'result-item';
        item.innerHTML = `
            <img src="${pl.image}" alt="pl" loading="lazy">
            <div class="result-info">
                <h4>${pl.name}</h4>
    ...
    // Tambah offline songs indicator
    if (offlineSongs.length > 0) {
    ...
    if(songs.length === 0) {
        ...
        songs.forEach((song, index) => {
            ...
            item.innerHTML = `
                ...
                <img src="${song.cover}" alt="art" loading="lazy">
                <div class="result-info">
                    <h4>${song.title}</h4>
                    <p>${song.artist}</p>
                </div>
